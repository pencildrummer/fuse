...
export function deleteProfile(id) {
  // Create path from id
  let profilePath = path.join(SYSTEM_BASE_PATH, 'profiles', id.replace('.', path.sep)+'.json')
  if (!fs.existsSync(path.join(process.cwd(), profilePath))) {
    throw new Error(`Requested deletion of profile with id ${chalk.magenta(id)} but no profile file is found at path ${chalk.magenta(profilePath)}`)
  }
  fs.unlinkSync(path.join(process.cwd(), profilePath))
  return true
}
