...
export default function TerminalWindow() {
  const { terminal, data, appendData, autoscroll, setAutoscroll } =
    useTerminalContext();

  const scrollArea = useRef<typeof ScrollArea>();
  ...
  function scrollToBottom() {
    ...
    if (scrollArea.current) {
      if (!scrollAreaViewport.current) {
        scrollAreaViewport.current = Array.from(
          scrollArea.current.children
        ).find((e) =>
          (e as HTMLElement).hasAttribute("data-radix-scroll-area-viewport")
        );
      }
