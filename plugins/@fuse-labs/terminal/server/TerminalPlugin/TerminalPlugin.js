  ...
  initDeviceSocket(socket, deviceId) {
    ...
    socket.on('open', (deviceId, fn) => {
      ...
      if (device.terminal.isOpen) {
        this._sendNamespaceMessage('Already connected', 'server', socket)
        return fn?.(true)
      } else {
        // Send message from server notify connection start process
        this._sendNamespaceMessage('Connecting to device...', 'server', socket)
    ...
    if (!device.terminal) {
      ...
      device.terminal = new DeviceTerminal(device, (err) => {
        ...
        if (err) {
          ...
          this._sendNamespaceMessage('Error connecting to device - ' + err.message, 'server', deviceNamespace)
      ...
      device.terminal.on('close', error => {
        if (error) {
          ...
          this._sendNamespaceMessage('Connection closed due to error: '+error.message, 'device', deviceNamespace)
        } else {
          signale.info('Closed device terminal connection for ', device.id, deviceNamespace.name)
          // Normal close request
          this._sendNamespaceMessage('Connection closed', 'device', deviceNamespace)
        }
      })
      ...
      device.terminal.on('error', error => {
        ...
        this._sendNamespaceMessage('Error: '+error.message, 'device', deviceNamespace)
      })

      signale.star('Added terminal to device ', chalk.bold(device.name), device.id)
    } else {
      console.info('Device already has a DeviceTerminal attached')
    }
  }
