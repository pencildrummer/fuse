...
class PluginManager {
  ...
  async loadSystemPlugin(systemPluginInfo) {
    ...
    let serverPath;
    if (typeof systemPluginInfo.path === "object") {
      serverPath = systemPluginInfo.path.server;
    } else {
      serverPath = systemPluginInfo.path;
    }
    ...
    const pluginModule = await import(`${systemPluginPath}`);
    //const pluginModule = await import(`${systemPluginInfo.name}`);
