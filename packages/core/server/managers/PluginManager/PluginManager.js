...
class PluginManager {
    ...
    async init() {
        ...
        this._plugins = await Object.keys(pluginsList).reduce(async (prev, pluginName) => {
    ...
    async loadSystemPlugin(systemPluginInfo) {
        ...
        // });
        // console.log("PKG", pkg);
        const systemPluginPath = path.resolve(path.join(process.cwd(), "../../", systemPluginInfo.path));
        ...
        try {
            ...
            console.error(`${chalk.bold.red(systemPluginInfo.name)} default export is not a class. Imported from ${importPath}`);
            return null;
        }
    ...
    async loadInstalledPlugin(pluginInfo) {
        ...
        if (!fs.existsSync(pluginPkgPath)) {
            return console.error("Unable to find package.json for plugin", pluginName);
        }
        // Read package.json
        let pluginPkg;
        try {
            ...
            return console.error("Error reading package.json from plugin", pluginName);
        }
        // Check if supports server side (should export server)
        if (pluginPkg.exports?.["./server"] == undefined) {
            return console.warn(`${chalk.yellow(pluginName)}: does not export a host plugin, skipping.`);
        }
