...
class ProfileManager {
    ...
    deleteProfile(profileId) {
        ...
        if (!fs.existsSync(profile.path)) {
            throw new Error(`Requested deletion of profile with id ${chalk.magenta(profileId)} but no profile file is found at path ${chalk.magenta(profile.path)}`);
        }
    ...
    _readProfile(profileId) {
        ...
        let profileData = fs.readJsonSync(path.join(process.cwd(), SYSTEM_BASE_PATH, "profiles", fileProfilePath + ".json"));
