...
import { Label, Tooltip, CompactList, Group } from "../../../shared";
...
export default function DeviceProfileList({
  ...
  return (
    <CompactList
      ...
      maxDepth={1}
      keyTransform={(key, value, isGroup) =>
        isGroup ? key : pathCase(value.model)
      }
...
function DeviceProfileListItem({ item, ...props }) {
  return (
    <CompactList.Item selectable {...props}>
      <Group orientation="vertical" className="flex-1">
        <Group className="flex-1">
          ...
          <div className="text-gray-500 hover:text-gray-300 transition-colors duration-150">
            <Tooltip
              content={<DeviceProfileTooltipContent profile={item} />}
              side="right"
              sideOffset={10}
              align="center"
              showArrow
...
function DeviceProfileTooltipContent({ profile }) {
  let infos = useMemo(() => {
    switch (profile.type) {
      case "fdm_printer":
        return {
          ...
          "Printing volume": [
            profile.volume.width,
            profile.volume.height,
            profile.volume.depth,
          ]
            .filter(Boolean)
            .join("x"),
